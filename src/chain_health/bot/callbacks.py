...
# Row ids are bounded, because the payload is client-forgeable and lands
# straight in a SQLite bind parameter. Python ints are unbounded, so a 23-digit
# value (comfortably inside Telegram's 64-byte limit) used to pass validation
# and then raise OverflowError deep inside the query — the user got the generic
